...
app.post("/api/admin/bot-join-group", requireAuth, requireFounder, async (req, res) => {
  ...
  const gid = parseInt(groupId, 10);
  try {
    // Grubun herkese açık olup olmadığını kontrol et
    const infoRes = await axios.get(`https://groups.roblox.com/v1/groups/${gid}`);
    if (!infoRes.data?.publicEntryAllowed) {
      return res.status(400).json({ error: "Bu grup herkese açık değil, bot katılamaz." });
    }
    // Önce X-CSRF-Token al
    let csrfToken = "";
    try {
    ...
    const joinRes = await axios.post(
      `https://groups.roblox.com/v1/groups/${gid}/users`,
      {},
      {
        headers: {
          "Cookie": `.ROBLOSECURITY=${cfg.ROBLOX_COOKIE}`,
          "X-CSRF-TOKEN": csrfToken,
          "Content-Type": "application/json"
        }
      }
    );
    console.log(`[bot-join-group] groupId=${gid} status=${joinRes.status}`);
    res.json({ ok: true });
  } catch(e) {
    const status = e.response?.status;
    const msg = e.response?.data?.errors?.[0]?.message || e.message || "";
    console.error(`[bot-join-group] HATA groupId=${gid}:`, status, msg);
    if (status === 400 && msg.toLowerCase().includes("already")) return res.status(400).json({ error: "Bot zaten bu grupta." });
    if (status === 403) return res.status(400).json({ error: "Captcha veya yetki engeli — bot bu gruba katılamıyor." });
    if (status === 429) return res.status(429).json({ error: "Çok fazla istek, biraz bekle." });
